...
import { cheatSheets, getResourceBySlug } from "@/lib/resources";

export function generateStaticParams() {
  return cheatSheets.map((resource) => ({ slug: resource.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const resource = getResourceBySlug("cheat-sheet", slug);
